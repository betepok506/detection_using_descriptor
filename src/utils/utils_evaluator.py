...
def add_bb_into_image(image, bb, color=(255, 0, 0), thickness=2, label=None):
    ...
    if label is not None:
        ...
        (tw, th) = cv.getTextSize(label, font, fontScale, fontThickness)[0]
        ...
        cv.rectangle(image, (r_Xin, r_Yin - thickness),
                     (r_Xin + tw + thickness * 3, r_Yin + th + int(12.5 * fontScale)), (b, g, r),
                     -1)
        cv.putText(image, label, (xin_bb, yin_bb), font, fontScale, (0, 0, 0), fontThickness,
                   cv.LINE_AA)
    return image
